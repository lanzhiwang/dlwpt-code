...
* linspace 	Creates a one-dimensional tensor of size steps whose values are evenly spaced from start to end, inclusive.
			创建一个 size steps 的一维张量，其值从 start to 到 （包括 to end ）均匀分布。
...
返回一个张量，其大小与 input 从泊松分布中采样的每个元素相同，其中 rate 参数由相应的元素给出， input 即
...
返回一个张量，其中填充了在 （含） 和 high （不含） 之间 low 均匀生成的随机整数。
...
返回一个与 Tensor 形状相同的张量， input 其中填充了在 （含） 和 high （不包括） 之间 low 均匀生成的随机整数。
...
torch.Tensor.bernoulli_() - 的就地版本 torch.bernoulli()

torch.Tensor.cauchy_() - numbers drawn from the Cauchy distribution
torch.Tensor.cauchy_() - 从 Cauchy 分布中抽取的数字
...
torch.Tensor.normal_() - 的就地版本 torch.normal()
